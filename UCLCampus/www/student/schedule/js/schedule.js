angular.module('ionicApp').controller('ScheduleController', function($scope, $rootScope, $cordovaCalendar, $ionicPopup, $http, $cookies, $timeout, $state) {
	...
	$scope.tomorrow= function(){
		...
		yesterday.setDate($scope.onezoneDatepicker.date.getDate()-1);
		$scope.onezoneDatepicker.date = yesterday;
	}
	$scope.yesterday= function(){
		var tomorrow = new Date();
		tomorrow.setDate($scope.onezoneDatepicker.date.getDate()+1);
		$scope.onezoneDatepicker.date= tomorrow;
	}
	...
	$scope.isToday = function(item){
		var day = $scope.onezoneDatepicker.date.getDate();
		if(day < 10){
			day = "0" + day;
		}
		var month = $scope.onezoneDatepicker.date.getMonth()+1;
		if(month < 10){
			month = "0" + month;
		}
		var year = $scope.onezoneDatepicker.date.getFullYear();
