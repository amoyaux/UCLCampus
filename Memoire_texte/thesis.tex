...
\author{Baptise \textsc{LACASSE}}	% Student name
...
The native container alleviates some the issues of the web approach but still reduces performances when compared to a native application. Table 2.3 shows the pros and cons of this final approach.
...
In the context of this thesis, there is no budget therefore choosing the native approach will only depend on the importance of accessing native features. There also won't be a web application, meaning the web approach is probably a bad choice.\\

 Furthermore, we believe that making it easier for outsiders to contribute is more important than accessing native features. Indeed, we don't think we will need any feature that would be supported by one platform but not the other and will mainly use generic features. Furthermore, the application will be updated every year (new lecture halls, libraries, ...). We therefore decided to go with an hybrid approach for this thesis.

\newpage
\subsection{Choosing the best hybrid framework}
There are many different hybrid frameworks and choosing one was no easy task. We were however able to find the following comparison of the most popular frameworks (source http://tutorialzine.com/2015/10/comparing-the-top-frameworks-for-building-hybrid-mobile-apps/)
...
Once we had a clear idea of what the application was going to look like, we were ready to start implementing it. We started by designing a high-level view of the program which is detailed in the "Architecture" section. This section details 

\section{Architecture}

The purpose of the application is to be extensible and easily maintainable. A programmer have the possibility to add his own functionalities at each level of the application.
The UCLCampus architecture can be seen as a tree. At the top level there are the generic parts and configurations that will be present everywhere in the application. Under it there are the settings menu and four other nodes representing the global sections. These sections are pointing to their functionalities and so on. Here is a part of the tree in order to give the idea.\\
...
To be consistent with our architecture, the folders and files organisation also follows a tree structure. Ionic base architecture is to group all html file in a folder named templates, all js in a JS folder, ... This design become messy once there are a lot of functionalities and a lot of files. We modify it to respect the same tree architecture that the implementation has. This folders organisation allows programmers to efficiency find a code they need. For example, a programmer want to change the cafeteria details user interface, he can follow the logic patch campus/cafeteria/cafeteriaDetails/templates. This folder path follows the tree implementation as it can be seen on figure 4.1. Figure 4.2 is a summary of the change compare to the base Ionic design. Red folders are those that have been modified. 
\begin{figure}
\centering
...
This section explains how to deal with informations processing from an architecture point of view. The section 4.4 explains in details how each specific part is retrieved.
There is a lot of informations to relay into the application (libraries schedule, libraries addresses, daily events in the campus,...). It exists three ways to import them. The first way is to use a database, the informations are encoded under table form and it can be retrieve due to query call. The two other methods are web parsing and web services. Web services are always better than web parsing. Web services consist to send a request to a server and receive a response that contains directly the informations. Web parsing send a request to a server but there is a need to search for the informations inside the response. The three methods are used in the application depending the case.\\ For example, the ADE schedule has no web services. It remains web parsing or database. Encode the schedule for all student in the database take 80Mo, it is more than the actual application size and it is too much for a mobile application. In this case, web parsing seems to be the right choice.\\ An other example, the events. Events are updated each day, there is not human resources to update the database each day. It remains web services and web parsing, web services are always better than web parsing and the louvainfo.be (website that contains events) provides it. So we selected web services.
...
Need human resources to update informations\par
Takes memory \par
...
Impossible if not implement on the server\par
...
Slow compare to web services and database\par
...
\item Back-end: Everything that is not part of user interface. Storage system, information retrieval.
\end{itemize}

The UCLCampus architecture contains a back-end/front-end system. It helps a lot for the modularity. For a functionality, the code for the user interface is totally isolated from the code for storage. That means, it is possible to change a user interface without seeing the code for storage. Inversely, it is possible to change the code for the storage without touching the user interface.\\
For example, it is possible to pass from a database storage to a web parsing for libraries without consider an impact on the user interface.

\subsection{Factory}

Factory is a functionality from angularJS that we used as a back-end service. The factory will take the data from the database or web parsing, modify the data format to be easier to handle and then transmit it to the front-end. Figure 4.3 illustrates this.\\
...
The state provider is a functionality of angularjs. It allows to define the page configuration (cached or not for example). Furthermore it create an edge between the back end and the front end, figure 4.4 illustrate the operation of the state provider. 
\begin{figure}[H]
\centering
